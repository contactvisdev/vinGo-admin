...
import { usePharmacyForm, weekDays } from './Container';
...
const PharmacyForm = () => {
  ...
  useEffect(() => {
    if (!isEdit && categoryList && categoryList.length > 0) {
      const pharmacyCategory = categoryList.find((cat) => cat.name.toLowerCase() === 'pharmacy');
      if (pharmacyCategory) {
        setValue('categoryId', pharmacyCategory._id);
      }
    }
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      if (isEdit) {
        ...
        navigate(`/merchant-management/pharmacies?tab=${tabFromUrl}&page=${page}&rows=${rows}`);
      } else {
  ...
  const breadcrumbs = [
    { label: 'Pharmacy', url: '/merchant-management/pharmacies' },
    {
      label: isEdit ? 'Edit Pharmacy' : 'Add Pharmacy',
  ...
  return (
    <MainCard>
      <div className="mb-4">
        <TopTitleCard title="Add Pharmacy" breadcrumbs={breadcrumbs} />
      </div>
