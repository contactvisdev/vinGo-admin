...
/**
 * Hook for handling the liquor list table, pagination, and loading state.
 */
export const useLiquorTable = () => {
  ...
  useEffect(() => {
    if (categoryList) {
      let categoryToSelect = categoryList?.find((cat) => cat.name.toLowerCase() === 'liquor')?._id;
  ...
  const onPageChange = (e) => {
    navigate(`/merchant-management/liquors?tab=${activeTab}&page=${e.page + 1}&rows=${e.rows}`);
    setFirst(e.first);
  };

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    navigate(`/merchant-management/liquors?tab=${tab}&page=1&rows=${rows}`, { replace: true });
  };
...
/**
 * Hook for handling the liquor form logic.
 */
export const useLiquorForm = () => {
...
/**
 * Hook for handling liquor deletion logic and confirmation modal.
 */
export const useLiquorDelete = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      console.error('Failed to delete liquor:', err);
    }
  };
