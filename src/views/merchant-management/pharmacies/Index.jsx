...
import { usePharmacyTable, usePharmacyDelete } from './Container';
    ...
    return (
      <div className="d-flex gap-2">
        <button
          ...
          onClick={() => navigate(`/merchant-management/pharmacies/view/${rowData._id}?tab=${activeTab}`)}
          title="view"
          disabled={loading}
        >
          <Eye size={16} />
        ...
        <button
          type="button"
          className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center"
          onClick={() => navigate(`/merchant-management/pharmacies/edit/${rowData._id}?tab=${activeTab}`)}
          title="Edit"
          disabled={loading}
        >
          <Pencil size={16} />
  ...
  return (
    <div className="container-fluid">
      ...
      <TabSection
        ...
        loading={loading}
        addButtonText="+ Add New Pharmacy"
        onAddClick={() => navigate('/merchant-management/pharmacies/add')}
        additionalButtons={
      ...
      {!loading && merchantList?.length === 0 ? (
        <div className="card shadow-sm p-4 d-flex justify-content-center align-items-center" style={{ height: '600px' }}>
          ...
          <p className="mt-3 text-muted">No merchants found</p>
        </div>
      ) : (
        <TableContainer
          list={merchantList}
      ...
      <ConfirmModal
        ...
        message={
          selectedMerchantsForDelete?.length > 1
            ? `Are you sure you want to delete ${selectedMerchantsForDelete.length} selected merchants?`
