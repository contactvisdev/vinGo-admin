...
import { useLiquorForm, weekDays } from './Container';
...
import { autoFillAddressFields, reverseGeocodeAndFill, buildCompleteAddress } from '../../../utils/addressParser';
...
const LiquorForm = () => {
  ...
  useEffect(() => {
    if (!isEdit && categoryList && categoryList.length > 0) {
      const liquorCategory = categoryList.find((cat) => cat.name.toLowerCase() === 'liquor');
      if (liquorCategory) {
        setValue('categoryId', liquorCategory._id);
      }
    }
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      if (isEdit) {
        ...
        navigate(`/merchant-management/liquors?tab=${tabFromUrl}&page=${page}&rows=${rows}`);
      } else {
      ...
      console.error('Failed to save liquor:', error);
    }
  };

  const breadcrumbs = [
    { label: 'Liquors', url: '/merchant-management/liquors' },
    {
      label: isEdit ? 'Edit Liquor' : 'Add Liquor',
      url: isEdit ? `/merchant-management/liquors/edit/${id}` : '/merchant-management/liquors/add'
    }
  ];
  ...
  return (
    <MainCard>
      <div className="mb-4">
        <TopTitleCard title="Liquors" breadcrumbs={breadcrumbs} />
      </div>
          ...
          <Form onSubmit={handleSubmit(onSubmit)} className={loading ? 'opacity-50 pe-none' : ''}>
            ...
            <Card className="mb-4 shadow-sm">
              ...
              <Card.Body>
                ...
                <Row>
                  <Col md={4}>
                    <TextInput name="business.location.plot_no" label="Plot No." placeholder="Enter plot number" />
                  </Col>
                  <Col md={4}>
