...
import { useLiquorTable, useLiquorDelete } from './Container';
    ...
    return (
      <div className="d-flex gap-2">
        <button
          ...
          onClick={() => navigate(`/merchant-management/liquors/view/${rowData._id}?tab=${activeTab}`)}
          title="view"
          disabled={loading}
        >
          <Eye size={16} />
        ...
        <button
          type="button"
          className="btn btn-outline-primary btn-sm rounded-circle d-flex align-items-center justify-content-center"
          onClick={() => navigate(`/merchant-management/liquors/edit/${rowData._id}?tab=${activeTab}`)}
          title="Edit"
          disabled={loading}
        >
          <Pencil size={16} />
  ...
  return (
    <div className="container-fluid">
      ...
      <TabSection
        ...
        loading={loading}
        addButtonText="+ Add New Liquor"
        onAddClick={() => navigate('/merchant-management/liquors/add')}
        additionalButtons={
      ...
      {!loading && merchantList?.length === 0 ? (
        <div className="card shadow-sm p-4 d-flex justify-content-center align-items-center" style={{ height: '600px' }}>
          ...
          <p className="mt-3 text-muted">No liquors found</p>
        </div>
      ) : (
        <TableContainer
          list={merchantList}
      ...
      <ConfirmModal
        ...
        message={
          selectedMerchantsForDelete?.length > 1
            ? `Are you sure you want to delete ${selectedMerchantsForDelete.length} selected liquors?`
