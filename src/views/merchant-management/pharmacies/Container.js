...
export const usePharmacyTable = () => {
  ...
  useEffect(() => {
    if (categoryList) {
      let categoryToSelect = categoryList?.find((cat) => cat.name.toLowerCase() === 'pharmacy')?._id;
  ...
  const onPageChange = (e) => {
    navigate(`/merchant-management/pharmacies?tab=${activeTab}&page=${e.page + 1}&rows=${e.rows}`);
    setFirst(e.first);
  };

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    navigate(`/merchant-management/pharmacies?tab=${tab}&page=1&rows=${rows}`, { replace: true });
  };
...
/**
 * Hook for handling the pharmacy form logic.
 */
...
/**
 * Hook for handling pharmacy deletion logic and confirmation modal.
 */
export const usePharmacyDelete = () => {
