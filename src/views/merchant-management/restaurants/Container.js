...
/**
 * Hook for handling the restaurant list table, pagination, and loading state.
 */
export const useRestaurantTable = () => {
  ...
  useEffect(() => {
    if (categoryList) {
      let categoryToSelect = categoryList?.find((cat) => cat.name.toLowerCase() === 'restaurant')?._id;
  ...
  const onPageChange = (e) => {
    navigate(`/merchant-management/restaurants?tab=${activeTab}&page=${e.page + 1}&rows=${e.rows}`);
    setFirst(e.first);
  };

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    navigate(`/merchant-management/restaurants?tab=${tab}&page=1&rows=${rows}`, { replace: true });
  };
...
/**
 * Hook for handling the restaurant form logic.
 */
...
/**
 * Hook for handling restaurant deletion logic and confirmation modal.
 */
